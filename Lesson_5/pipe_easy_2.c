...
int fill_command(char* command[], int numb, struct str* text, int pipefd[][2], int word_counter, int out, int* pipe_counter);
...
int main(int argc, char** argv)
    ...
    word_counter = fill_command(command, numb, text, pipefd, word_counter, out, &pipe_counter);
    int fork_code = fork();
    if (fork_code == 0)
        {
        //char write_buf[100] = {'\0'};
        //sprintf(write_buf, "%s%s %s %s word_counter = %d\n", "execvp = ", command[0], command[1], command[2], word_counter);
        //write(out, write_buf, 100); 
...
int fill_command(char* command[], int numb, struct str* text, int pipefd[][2], int word_counter, int out, int* pipe_counter)
    ...
    //char write_buf[100] = {'\0'};
    //sprintf(write_buf, "%s%s %s %s word_counter = %d\n", "in_ fill_command  execvp = ", command[0], command[1], command[2], word_counter);
    //write(out, write_buf, 100);  
